(function() {
    ...
    function getActiveTab() {
        const tab = localStorage.getItem('stash-copilot-active-tab') || 'summary';
        // The 'train' tab was removed (ADR-0001). Fall back to 'summary' for any
        // stale saved value so the modal always opens on a real panel.
        if (tab === 'train') return 'summary';
        return tab;
    }
    ...
    function switchInsightsTab(modal, tabName) {
        ...
        if (tabName === 'summary' || tabName === 'chat' || tabName === 'tools' || tabName === 'taste_map' || tabName === 'tag_gaps') {
            modal.removeAttribute('data-empty');
        }
...
• <strong>Frames</strong>: Number of frames this tag would help cover`
    ...
    function init() {
        ...
        // Setup handler for browser back/forward navigation from the search page
