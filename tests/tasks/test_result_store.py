...
def test_save_writes_under_result_key_and_request_id(tmp_path: Path) -> None:
    ...
    path = store.save("sample_task", "req-42", result)

    assert path == tmp_path / "sample_task_req-42.json"
...
def test_load_round_trips_the_saved_result(tmp_path: Path) -> None:
    ...
    store.save("sample_task", "abc", result)

    assert store.load("sample_task", "abc") == result
...
def test_save_skips_when_request_id_empty(tmp_path: Path) -> None:
    ...
    path = store.save("sample_task", "", {"status": "complete"})
...
def test_save_creates_assets_dir_when_missing(tmp_path: Path) -> None:
    ...
    path = store.save("other_task", "r1", {"ok": True})

    assert assets_dir.is_dir()
    assert path == assets_dir / "other_task_r1.json"
...
def test_path_for_does_not_write(tmp_path: Path) -> None:
    ...
    path = store.path_for("sample_task", "req-1")

    assert path == tmp_path / "sample_task_req-1.json"
...
def test_save_accepts_string_assets_dir(tmp_path: Path) -> None:
    ...
    path = store.save("sample_task", "s1", {"value": 1})

    assert path == tmp_path / "sample_task_s1.json"
    assert path is not None and path.exists()
